...
import { WasmRect } from 'wasm-ulmo-map';

function strictParseInt(int, errMessage) {
  let num = Number.parseInt(int, 10);
  if (isNaN(num)) {
    throw new Error(`${errMessage} ${int}`);
  }
  return num;
}

function strictParseFloat(float, errMessage) {
  let num = Number.parseFloat(float);
  if (isNaN(num)) {
    throw new Error(`${errMessage} ${float}`);
  }
  return num;
}
...
export function getDrawingContext(canvas, options) {
...
export function parseLevel(levelStr) {
  if (levelStr.startsWith('S')) {
    let level = strictParseFloat(levelStr.substring(1),
  ...
  if (levelStr.startsWith('D')) {
    let levels = levelStr.substring(1).split('-');
    if (levels.length !== 2) {
      throw new Error(
          `Down level did not have a level and drop component separated by '-': ${levelStr}`);
    }
    let levelVal = strictParseInt(levels[0], 'Level component could not be parsed as an int:');
    let dropVal = strictParseInt(levels[1], 'Drop component could not be parsed as an int:');
    if (dropVal < 1) {
      throw new Error('Drop component was not greater than zero');
    }
    return {
      type: 'down',
      level: levelVal,
