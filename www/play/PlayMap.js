...
import { WasmPlayMap } from 'wasm-ulmo-map';
...
class PlayMap {
    ...
    applyMove(mxIn, myIn, levelIn, baseRect) {
        const { valid, deferral, level, mx, my } = this.wasmPlayMap.applyMove(mxIn, myIn, Math.round(levelIn * 2), baseRect.toWasmRect());
    ...
    getEvent(level, baseRect) {
        const event = this.wasmPlayMap.getEvent(Math.round(level * 2), baseRect.toWasmRect());
        if (event) {
