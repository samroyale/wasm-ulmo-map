...
class MapTile {
  ...
  _initImageData() {
    let tileCanvas = drawTile(this._maskTiles, this._baseTileCanvas);
  ...
  //     return;
  //   }
  //   let topTile = this._maskTiles.pop();
...
class RpgMap {
  ...
  //   let xBound = Math.min(tiles.length, this.getCols() - x);
  //   let yBound = Math.min(tiles[0].length, this.getRows() - y);
  //   let oldTiles = this._copyInternal(x, y, yBound, xBound);
  ...
  //   let xBound = tiles.length, yBound = tiles[0].length;
  //   for (let i = 0; i < xBound; i++) {
  //     for (let j = 0; j < yBound; j++) {
  //       this.applyTile(
  //         this._mapTiles[i + x][j + y],
  ...
  //     y: 0 - Math.min(0, top)
  //   };
  //   let rows = rpgMap.getRows() - sourceTopLeft.y;
  //   let cols = rpgMap.getCols() - sourceTopLeft.x;
  //   let targetTopLeft = {
  //     x: Math.max(0, left),
  //     y: Math.max(0, top)
  //   };
  ...
  //   let oldTile = this._mapTiles[x][y].copy();
  //   this._mapTiles[x][y].setMaskTiles(maskTiles);
  //   return [[oldTile]];
  // };
  //
  // _applyMutation = (topLeft, rows, cols, func) => {
  //   let oldTiles = new Array(cols);
  //   for (let i = 0; i < cols; i++) {
  //     oldTiles[i] = new Array(rows);
  //     for (let j = 0; j < rows; j++) {
  //       let mapTile = this._mapTiles[i + topLeft.x][j + topLeft.y];
  ...
  //       let mapTile = this._mapTiles[x][y].getDto(x, y);
  //       if (mapTile) {
  //         mapTiles.push(mapTile)
  //       }
  //     }
  //   }
  //   let sprites = this._sprites.map(sprite => sprite.getDto());
  //   return {
  //     name: name,
  //     rows: rows,
  //     cols: cols,
...
class RpgMapService {
  ...
  async loadMap(mapId) {
    try {
      const response = await fetch(`${rpgMapsApi}/${mapId}.json`, { method: 'GET', cache: 'no-store' });
      if (response.ok) {
        const json = await response.json();
  ...
  _buildRpgMap(rpgMapDef, tileSetsArray) {
    ...
    // deferred.notify(100);
    return new RpgMap(uuidv4(), rpgMapDef.name, mapTiles, sprites);
  };
  ...
  _initMapTiles(rpgMapDef, tileSets) {
    ...
    for (let x = 0; x < cols; x++) {
      ...
      for (let y = 0; y < rows; y++) {
        ...
        if (mapTileDef) {
          let maskTiles = mapTileDef.tiles.map(tileDef => {
